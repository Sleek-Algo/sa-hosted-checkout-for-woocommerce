...
trait Request
{
     ...
     *
     * @throws \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException if $params exists and is not an array
     */
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            $message = 'You must pass an array as the first argument to SAHCFWC\Libraries\Stripe API '
                . 'method calls.  (HINT: an example call to create a charge '
                . "would be: \"SAHCFWC\Libraries\Stripe\\Charge::create(['amount' => 100, "
                . "'currency' => 'usd', 'source' => 'tok_1234'])\")";

            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    ...
    /**
     * @param string $url URL for the request
     * @param class-string< \SAHCFWC\Libraries\Stripe\SearchResult|\SAHCFWC\Libraries\Stripe\Collection > $resultClass indicating what type of paginated result is returned
     ...
     * @throws \SAHCFWC\Libraries\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \SAHCFWC\Libraries\Stripe\Collection|\SAHCFWC\Libraries\Stripe\SearchResult
     */
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        if (!($obj instanceof $resultClass)) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\UnexpectedValueException(
    ...
    {
        $opts = \SAHCFWC\Libraries\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \SAHCFWC\Libraries\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
