...
use SAHCFWC\Libraries\Stripe\Util\Util;
...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'stripe_account' => null,
        'stripe_context' => null,
        'stripe_version' => \SAHCFWC\Libraries\Stripe\Util\ApiVersion::CURRENT,
    ...
    /** @var \SAHCFWC\Libraries\Stripe\Util\RequestOptions */
    private $defaultOpts;
     ...
     *
     * - api_key (null|string): the SAHCFWC\Libraries\Stripe API key, to be used in regular API requests.
     * - app_info (null|array): information to identify a plugin that integrates SAHCFWC\Libraries\Stripe using this library.
     *                          Expects: array{name: string, version?: string, url?: string, partner_id?: string}
     * - client_id (null|string): the SAHCFWC\Libraries\Stripe client ID, to be used in OAuth requests.
     * - stripe_account (null|string): a SAHCFWC\Libraries\Stripe account ID. If set, all requests sent by the client
     *   will automatically use the {@code Stripe-Account} header with that account ID.
     * - stripe_context (null|string): a SAHCFWC\Libraries\Stripe account or compartment ID. If set, all requests sent by the client
     *   will automatically use the {@code Stripe-Context} header with that ID.
     * - stripe_version (null|string): a SAHCFWC\Libraries\Stripe API version. If set, all requests sent by the client
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \SAHCFWC\Libraries\Stripe\Util\RequestOptions::parse([
     ...
     *
     * @return null|array information to identify a plugin that integrates SAHCFWC\Libraries\Stripe using this library
     */
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $apiMode = \SAHCFWC\Libraries\Stripe\Util\Util::getApiMode($path);

        $opts = $defaultRequestOpts->merge($opts, true);

        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \SAHCFWC\Libraries\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl, $this->getAppInfo());
        list($response, $opts->apiKey) = $requestor->request($method, $path, $params, $opts->headers, $apiMode, ['stripe_client']);
        $opts->discardNonPersistentHeaders();
        $obj = \SAHCFWC\Libraries\Stripe\Util\Util::convertToStripeObject($response->json, $opts, $apiMode);
        if (\is_array($obj)) {
            // Edge case for v2 endpoints that return empty/void response
            // Example: client->v2->billing->meterEventStream->create
            $obj = new \SAHCFWC\Libraries\Stripe\StripeObject();
        }
     ...
     * with the SAHCFWC\Libraries\Stripe API. This method is useful for interacting with endpoints that are not
     ...
     *
     * @return \SAHCFWC\Libraries\Stripe\ApiResponse
     */
    public function rawRequest($method, $path, $params = null, $opts = [])
    {
        ...
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \SAHCFWC\Libraries\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
     ...
     * @param array|\SAHCFWC\Libraries\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \SAHCFWC\Libraries\Stripe\Collection|\SAHCFWC\Libraries\Stripe\V2\Collection of ApiResources
     */
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        $apiMode = \SAHCFWC\Libraries\Stripe\Util\Util::getApiMode($path);
        if ('v1' === $apiMode) {
            if (!($obj instanceof \SAHCFWC\Libraries\Stripe\Collection)) {
                $received_class = \get_class($obj);
                $msg = "Expected to receive `SAHCFWC\Libraries\Stripe\\Collection` object from SAHCFWC\Libraries\Stripe API. Instead received `{$received_class}`.";

                throw new \SAHCFWC\Libraries\Stripe\Exception\UnexpectedValueException($msg);
            }
            $obj->setFilters($params);
        } else {
            if (!($obj instanceof \SAHCFWC\Libraries\Stripe\V2\Collection)) {
                $received_class = \get_class($obj);
                $msg = "Expected to receive `SAHCFWC\Libraries\Stripe\\V2\\Collection` object from SAHCFWC\Libraries\Stripe API. Instead received `{$received_class}`.";
    ...
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \SAHCFWC\Libraries\Stripe\SearchResult)) {
            $received_class = \get_class($obj);
            $msg = "Expected to receive `SAHCFWC\Libraries\Stripe\\SearchResult` object from SAHCFWC\Libraries\Stripe API. Instead received `{$received_class}`.";
    ...
    /**
     * @param \SAHCFWC\Libraries\Stripe\Util\RequestOptions $opts
     *
     * @throws \SAHCFWC\Libraries\Stripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \SAHCFWC\Libraries\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }

        if (null !== $config['api_key'] && ('' === $config['api_key'])) {
            $msg = 'api_key cannot be the empty string';

            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_context
        if (null !== $config['stripe_context'] && !\is_string($config['stripe_context'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('stripe_context must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }

        // app info
        if (null !== $config['app_info'] && !\is_array($config['app_info'])) {
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('app_info must be an array');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
     ...
     * @param 'v1'|'v2' $apiMode
     *
     * @return \SAHCFWC\Libraries\Stripe\StripeObject
     * */
    public function deserialize($json, $apiMode = 'v1')
    {
        return \SAHCFWC\Libraries\Stripe\Util\Util::convertToStripeObject(\json_decode($json, true), [], $apiMode);
    }
     ...
     *
     * @param string $payload the payload sent by SAHCFWC\Libraries\Stripe
     ...
     *
     * @return \SAHCFWC\Libraries\Stripe\ThinEvent
     */
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object(
                $eventData,
                '\SAHCFWC\Libraries\Stripe\ThinEvent'
            );
        } catch (\ReflectionException $e) {
            // Fail gracefully
            return new \SAHCFWC\Libraries\Stripe\ThinEvent();
        }
    }
}
