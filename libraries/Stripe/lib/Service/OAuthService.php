...
class OAuthService extends \SAHCFWC\Libraries\Stripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \SAHCFWC\Libraries\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \SAHCFWC\Libraries\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \SAHCFWC\Libraries\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \SAHCFWC\Libraries\Stripe\StripeClient([clientId => <CLIENT-ID>
                ])`)".  You can find your client_ids '
              . 'in your SAHCFWC\Libraries\Stripe dashboard at '
     ...
     *
     * @return \SAHCFWC\Libraries\Stripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \SAHCFWC\Libraries\Stripe\Service\OAuthService from \SAHCFWC\Libraries\Stripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \SAHCFWC\Libraries\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \SAHCFWC\Libraries\Stripe\Util\RequestOptions::parse($opts);
    }
