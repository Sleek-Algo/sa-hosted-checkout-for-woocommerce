...
use SAHCFWC\Libraries\Stripe\StripeObject;
...
abstract class Util
{
    ...
    /**
     * Converts a response from the SAHCFWC\Libraries\Stripe API to the corresponding PHP object.
     *
     * @param array                $resp    the response from the SAHCFWC\Libraries\Stripe API
    ...
    {
        $types = 'v1' === $apiMode ? \SAHCFWC\Libraries\Stripe\Util\ObjectTypes::mapping
            : \SAHCFWC\Libraries\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    $eventTypes = \SAHCFWC\Libraries\Stripe\Util\EventTypes::thinEventMapping;
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        $class = $eventTypes[$resp['type']];
                    } else {
                        $class = \SAHCFWC\Libraries\Stripe\StripeObject::class;
                    }
                }
                ...
                $class = \SAHCFWC\Libraries\Stripe\V2\Collection::class;
            } else {
                $class = \SAHCFWC\Libraries\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \SAHCFWC\Libraries\Stripe\RelatedObject();
                    ...
                    $reason = new \SAHCFWC\Libraries\Stripe\Reason();
    ...
    {
        if ($h instanceof \SAHCFWC\Libraries\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
